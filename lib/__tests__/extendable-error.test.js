import { ExtendableError, matchError } from "../extendable-error";
...
class HerError extends ExtendableError {
    static get matchers() {
        return { name: [ /^HerError/ ] };
    }
}

class TheirError extends ExtendableError {
    static get matchers() {
        return { message: [ /^their mistake/ ] };
    }
}
...
describe( "extendable-error", () => {
    ...
    describe( "matchError", () => {
        ...
        it( "works", function stackTestFunction() {
            const errorsDict = {
                MyError: MyError,
                HerError: HerError,
                TheirError: TheirError
            };

            expect( matchError( new MyError( "this is test" ), errorsDict ) ).toEqual( undefined );
            expect( matchError( new HerError( "her mistake" ), errorsDict ) ).toBe( HerError );
            expect( matchError( new Error( "their mistake" ), errorsDict ) ).toBe( TheirError );
        } );
    } );
